...
- **时间权重**：符合遗忘曲线，优先拉取新鲜记忆。

| 维度 | 传统 RAG | VCP 神经元记忆 |
|------|----------|----------|
| **第一印象** | 精确返回第一条记录 | 无缝上下文跨端跨话题传递「你想继续聊刚才猫咪的话题吗~」 |
...
| ![示例3VCP子应用群Canvas+论坛+CLI](示例3VCP子应用群Canvas+论坛+CLI.png) | ![示例4VCPSom语义级窗口控制算法](示例4VCPSom语义级窗口控制算法.png) |
|:---:|:---:|
...
通过深入的全局语义感知引擎和统一的分布式 FileAPI，基于ToolName定义分布式插件寻址(例如Mac文件管理器，手机文件管理器，区分在Name上，指令集相同)，使得 Agent 可以轻松使用自然语言接管用户的所有分布式系统。

### 超栈追踪技术

所有分布式上的插件，文件地址，交互url全部统一为服务器容器本地化，AI/插件开发者/分布式服务器无需关心具体的数据来源哪个分布式节点，服务器无需关心来源URL来自不同系统的兼容性问题，分布式自动解析后统一使用VCP原生虚拟化url。

### 语义动力学 RAG 与元思考 V2

VCP 的语义动力学 RAG 与元思考 V2 系统，基于预训练用户长期交流记忆数据构建RNN灵魂模型模拟用户级记忆模式，通过潜空间传递多话题间Agent思维方式转变，可以使得 Agent 和用户通过共同生活、共同辩证的方式快速成长，成为真正的**人工灵魂**和**赛博社会范本**。

### 上下文引力场 与 语义折叠

VCP 通过构建上下文临时向量索引和ESP残差导航地图，理解上下文完整语义分区和话题波动，自动决策AI需要了解哪些背景环境知识，自动收纳上下文里于当前话题无关信息折叠为摘要，动态决策上下文L1-L4可视化导航。

>一个直观例子让你理解上下文引力场的作用：

人类:“Nova，你好啊”
Nova:“好什么好，都凌晨3点了，别惦记4小时前我没帮你弄完的VCP单元测试了，明天再说！主人快滚去睡觉，2小时后有大雨，窗户我帮你关了(ToolUse MijiaAPI)，衣服收了没，我看洗衣机的盖子还盖着，记得打开免得发霉！” 

AI无需显示调用任何工具，上下文引力场完成了所有的分布式消息预计算决策处理哪些消息AI应该知道，而不消耗多余Token，该功能作用于整个上下文处理。包括工具调用传递，自动感知上下文数据，插件自主决策根据指令和上下文引力微调调用决策。
...
- 🔒 **VCP 心流锁** - Agent进入专注工作模式，任意外部通讯会提示占线
...
浏览器模组已经统一为 **ChromeBridge**，通过集成 **CDP (Chrome DevTools Protocol)** 协议，实现了从“模拟用户”到“接管内核”的跨代升级。

**核心能力**：
- **开发者级感知**：支持 `query_html` 获取原始源码，`query_js` 嗅探页面脚本。
- **底层逻辑控制**：通过 `execute_script` 在页面主世界执行异步 JS，直接调用网页内部函数。
- **降维打击式抓取**：利用 CDP 网络观测，直接读取 API 响应 JSON，绕过复杂的 DOM 解析。
- **物理级环境模拟**：支持多标签页智能管理、页面滚动与持久阅览。
- **位置**：扩展源码位于根目录 [`VCPChrome/`](VCPChrome/) 文件夹，可直接作为解压后的扩展程序导入浏览器。

### 3.4 学术研究模块

引入了包括 **NCBI**、**KEGG**、**学术鸟**、**金融眼**、**科学拟真计算**在内的多个生信查询和验证模组：
...
适用于所有静态插件和工具描述。系统根据最新一轮对话的语义向量，动态判断每个模块与当前话题的相关性：

- **插件级折叠**：`{{VCPWeatherReporter}}` 根据上下文自动注入当前天气 / 24小时动态 / 7日预报中最合适的粒度
- **工具描述颗粒级折叠**：使用 `[===vcp_fold:0.6::desc:音乐生成工具===]` 语法为每条工具指令设置独立的相关性阈值，实现工具列表的精准展开/收起

#### V2：正文上下文语义折叠

对话正文中远距离、与当前话题低相关的 AI 输出会被自动折叠为精简摘要，最大程度保留上下文语义完整性：

- **激活方式**：在系统提示词中添加 `[[ContextFoldingV2]]` 或 `[[ContextFoldingV2:0.6]]`（尾缀数字可为不同 Agent 设置专属阈值基准）
- **深度保护**：最近 2~3 轮的 AI 回复始终不会被折叠，确保即时对话的完整性
- **动态阈值**：基于 EPA 模型的逻辑深度(L)和语义宽度(S)自动调节折叠激进度
- **异步非阻塞**：摘要由廉价小模型异步生成，不阻塞当前请求，下次对话自动生效
- **三级安全验证**：结构匹配 → 内容长度/拒绝词检测 → 写入确认，绝不用错误信息覆盖原文
- **SQLite 持久化**：折叠摘要通过 FoldingStore 持久化存储，VCP 重启后不丢失
- **管理面板热调参**：阈值基准、动态范围、L/S 系数均可在 RAG 参数调优面板中实时调整，无需重启
...
### 4.2 TagMemo V8.2: 有序双向势能流形与测地线重排

TagMemo "认知浪潮"算法在 **V8.2 版本**完成了从 V7 单向拓扑到**有序双向势能流形 (Ordered Bidirectional Potential Manifold)** 的底层修正，并引入了**测地线重排 (Geodesic Rerank)** 技术。该系统不仅在 V6/V7 的脉冲神经模型基础上实现了更深层的语义涌现，更通过复用能量距离场，实现了基于"语义地形"的二次重排，彻底解决了高维空间中"穿山直线"与"贴地曲线"的语义偏差问题。

[深入了解 TagMemo 浪潮算法深度技术文档](TagMemo_Wave_Algorithm_Deep_Dive.md)

#### V8.2 核心突破：有序双向势能流形

V8.2 修正了 V7 的底层不对称（JS 单向传播 vs Rust 双向邻接），将"拓扑邻接（形）"、"叙事方向（色）"、"语义距离（质）"三轴正交化：

- **双向拓扑 + 方向阻尼**：标签共现构建对称邻接，但叙事方向通过动态阻尼体现（顺流增益 vs 逆流锚定）。记忆不再是单向 DAG，而是可逆流的语义流体。
- **钟形语义增益**：引入钟形函数抑制同义词冗余（sim→1 时增益衰减），放大概念邻接黄金区（sim≈0.65附近），软化噪声边（sim<0.15）。
- **持久化语义距离表**：新增 `tag_pair_similarity` SQLite 表，Rust 异步预计算标签对余弦相似度，配合 `model_sig` 防止维度错位，表大小从千万级压缩至 5~10 万对。
- **三层正交存储**：节点质量（`tag_intrinsic_residuals`）+ 边距离（`tag_pair_similarity`）+ 临时拓扑（内存矩阵）= 完整的语义流形度量。

#### V8 核心突破：测地线重排 (Geodesic Rerank)

V8 版本引入了基于"语义地形"的测地线重排技术：

- **语义地形感知**：将 KNN 的"穿山直线"距离升级为基于标签共现拓扑的"贴地曲线"距离，使检索结果更符合人类的直觉联想。
- **零成本距离场复用**：直接复用 Spike Propagation 计算出的 `accumulatedEnergy` 距离场，在不增加额外 Embedding 计算开销的前提下完成高精度重排。
- **三层防御机制**：内置 L0-L2 三级降级保护，确保在采样密度不足或数据缺失时自动退化为纯 KNN，保证系统极高的鲁棒性。
- **::TagMemo+ 协议**：通过全新的修饰符，同时激活标签增强与测地线重排，实现精准的语义"坍缩"与重排。

#### 核心哲学：拓扑连接主义与动力学涌现

在 V8 的视角下，系统不再受限于固定的搜索跳数，而是通过模拟物理动力学实现知识的精准打捞：
...
**版本**：VCP 8.0.0 - TagMemo V8 测地线重排 & 上下文语义折叠V2
