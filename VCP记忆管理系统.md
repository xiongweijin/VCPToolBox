...
-   **优势**：即使是能力较弱的模型驱动的 Agent，或在高负载下偶尔"偷懒"的 AI，也能确保日记库的 Tag 体系始终保持高质量和一致性，为后续的检索和分析提供坚实基础。
	...
	### 3. `DailyNoteManager`日记管理器插件（hybridservice混合插件）
	...
	**推荐使用方式**：不推荐Agent自己操作该插件。建议使用VCP服务器内置的记忆大师Agent来操作该插件。只推荐Claude 4.5 Opus, Gemini 3 Pro, GPT 5 Ultra来驱动该Agent，避免低级模型语料污染高级模型。
	...
	### 3.1 梦系统V2
	`AgentDream`插件中，Agent将通过三级驻波种子放电深度记忆交互系统自动构建逻辑脉络来进行记忆整理，详见梦系统技术文档。
	...
	VCP 内置了一套基于**语义相似度**的**提示词动态折叠/展开**系统，专门用于管理大型插件指令集。它解决了一个核心痛点：Agent 可能拥有数十个工具插件，但每个工具的完整说明文档可能有数千 token——如果全部注入系统提示词，会严重浪费上下文窗口。

	-   **核心理念**：将所有工具说明按功能分组收纳在"收纳箱"（Toolbox）中，只有当用户的对话上下文与某个工具组的**描述语义**足够接近时，才自动展开该组内容。其余工具组保持折叠状态，仅显示一行提示告知 Agent"还有 N 个工具模块隐藏中"。
	...
	在每个收纳箱的 `.txt` 文件内部，使用 `[===vcp_fold:===]` 标记来划分内容区块，每个区块可以独立设置**展开阈值**和**语义描述**：
	...
	2.  **双层判定**：
	    ...
	    -   **区块级**（`toolbox_block_similarity` 策略）：如果区块有独立的 `::desc:` 描述，则用该描述独立计算与上下文的相似度，只有达到阈值的区块才展开。
	3.  **缓存优化**：描述向量通过 SQLite `kv_store` 持久化缓存（由 KnowledgeBaseManager 的 `getPluginDescriptionVector` 管理），首次计算后不再重复调用 Embedding API。
	4.  **热重载**：`toolbox_map.json` 和 `TVStxt/` 目录下的文件均受 chokidar 监听，修改后自动清除缓存并重新加载，无需重启服务器。
...
- **未来的基石**: 这一坚实的底层架构，为未来 `RagTagMemo` 系统的进一步深入优化和功能拓展打下了不可撼动的基石，确保了 VCP 记忆系统长期、高效、稳定地进化。
	...
	*   **Rust驱动的内存安全与极致性能**: 核心逻辑由Rust构建，引入了原创的语义级Chunk去重函数，杜绝内存安全问题，确保系统长期稳定并完全释放硬件性能。
	...
	-   **TimeLine系统**：使用{{小克TimeLine}}，即可让小克看到所有“小克编写的日记”的时间戳和标题，构建真正的时间线认知。该系统由记忆大师Agent自动维护，合并多个连续事件标题。目前该系统的管理GUI已经迁移到独立项目。[https://github.com/lioensky/VCPTimeLine]
	...
	**Expand** 解决了长日记被切分为多个 chunk 后，RAG 只召回片段而非完整日记的问题。启用后，只要命中任意 chunk，系统就会自动展开为该 chunk 所属的**完整日记文件**内容。
	...
	**Associate** 为日记本检索引入了一种全新的认知拓展能力：**潜在共现发现**。它将已召回的每个 chunk 作为独立种子，反向搜索索引，找出被多个种子同时"联想到"的隐藏知识——这些知识可能与原始查询的直接语义距离较远，但在记忆网络的拓扑结构中被多条不同的思维路径交叉关联。
	...
	-   **工作逻辑**：
	    ...
	    2.  **种子联想**：每个已召回的 chunk 作为独立种子，以当前动态 K 值在目标索引中执行**纯语义搜索**（tagBoost=0，不施加 TagMemo 偏置），产生 n 组联想结果。
	    3.  **共现筛选**：从 n 组结果中提取**至少被 2 个不同种子联想到**的 chunk，作为"潜在认知共现"。
	    4.  **额外追加**：共现结果作为**额外内容**追加到原始召回结果之后，不占用原始 K 配额。例如动态 K=6 召回了 6 条主结果，联想发现了 3 条共现，最终输出 9 条。
	    5.  **Expand 兼容**：如果同时使用了 `::Expand`，联想共现结果也会被正确展开为完整日记文件。`::Expand` 始终在 `::Associate` 之后执行。
	-   **聚合模式下的跨域联想**：
	    -   在聚合检索语法（`|` 分隔多日记本）中，每个种子 chunk 会跨**所有聚合日记本索引**执行联想搜索，而非仅在自己所属的索引内搜索。
	    -   这实现了真正的**跨域认知共现**：来自"物理"日记本的一条记忆和来自"哲学"日记本的一条记忆，可能同时联想到了"政治"日记本中的某条隐藏关联。
	...
	**Base64Memo** 为日记本检索引入了跨模态感知能力。它允许系统自动从召回的日记正文中提取多媒体附件，并将其以 Base64 格式注入到当前对话中。
	...
	-   **工作模式**：
		...
		3.  连续认知优化，检测到话题切换后自动续接上一次对话的记忆衔接问题。
		4.  **支持多时间点查询**：如果一句话中包含多个时间点（如“我和小克上周以及三个月前都聊了什么？”），插件会分别解析并合并所有时间范围的结果。
		5.  **智能去重**：算法会自动对解析出的时间范围和最终检索到的日记内容进行去重，确保结果的简洁性和唯一性。
		6.  **混合检索**：最终结果会智能融合“语义相关（RAG）”和“时间范围”两个维度的信息，提供最全面的上下文回溯。
...
`::TagMemo+` 是 TagMemo 的增强模式（V8），它在标准 TagMemo 的基础上引入了**测地线重排**——利用脉冲扩散过程中已计算的**能量距离场**，对 KNN 候选结果进行基于"认知拓扑地形"的二次精排。

-   **核心思想**：标准的 KNN 搜索只考虑向量空间中的欧氏/余弦距离（直线距离），但知识之间的真实关联往往不是直线的。测地线重排将 TagMemo 脉冲传播（LIF 神经元模型）产生的**能量场**作为"地形图"，计算每个候选 chunk 与当前认知焦点之间的**地形贴地距离**，从而发现那些"向量距离一般，但在认知拓扑上高度关联"的记忆。
...
-   **工作流程**：
    ...
    4.  **地形评分**：计算每个 chunk 关联标签在能量场中的**平均能量值**，作为该 chunk 的测地线分数（`geoScore`）。
...
核心优势：通过AI的深度推理能力，能够发现传统向量检索无法捕捉的复杂逻辑关系、隐含因果链和跨时间线的事件关联，实现真正"智能"的记忆召回。虽然提高了回忆成本，但更适合逻辑高度复杂的大型记忆/知识库，返回真正精准的结果。
	...
	### AIMemo+ TagMemo初筛模式 (TagMemo-First AIMemo) - 巨型知识库的精明AI召回

	`::AIMemo+` 是 `::AIMemo` 的进化形态，专为 **巨型记忆/知识库（数百MB至GB级）** 场景设计。它在保留 AIMemo "AI军团并发推理"核心优势的同时，引入 TagMemo 向量级初筛，将 LLM 看到的内容从"整本日记"压缩为"最相关的认知拓扑切片"，token 消耗降低一个数量级，且召回精度不降反升。

	-   **启用语法**：在 `[[]]` 或 `《《》》` 模式后添加 `::AIMemo+` 标记：
	    ```
	    [[角色日记本::AIMemo+]]                  ← 单库 Plus 模式
	    [[角色日记本::AIMemo+:custom_preset]]    ← Plus + 自定义预设
	    [[A|B|C日记本::AIMemo+]]                 ← 跨库聚合 Plus
	    [[A|B|C日记本::AIMemo+:1.5]]             ← 跨库聚合 + K倍率（initSearchK = dynamicK × 5 × 1.5）
	    《《知识库::AIMemo+》》                    ← 混合模式 Plus（受相似度阈值门控）
	    ```
	-   **激活前提**：与 `::AIMemo` 一致，需要在 system 提示词中携带 **AIMemo 许可证** `[[AIMemo=True]]`，否则该修饰符不生效（自动回退为标准 RAG）。

	-   **工作流程**：
	    1.  **TagMemo 初筛**：使用当前查询向量、动态 Tag 权重和幽灵节点（`[@tag]`），通过 V6 浪潮算法在所有指定日记本中执行**残差金字塔感应** + **N-hop 脉冲扩散**搜索，召回 `K = dynamicK × 5` 个最相关 chunks。
	    2.  **跨库去重**：按文本指纹去重并按相关度降序排列，形成"虚拟知识库"。
	    3.  **AI 军团推理**：将虚拟知识库（而非整本日记）打包送入 AIMemo 模型，由轻量级推理模型提取 `[[...]]` 记忆块。
	    4.  **批次保护**：如果初筛后的 chunks 仍然超出 `AIMemoMaxTokensPerBatch` 限制，自动启用分批处理流水线（与标准 AIMemo 一致）。
	    5.  **跨批合并**：多批结果按 `[[...]]` 区块去重合并，输出统一的"跨库联合检索"结果。

	-   **与 `::AIMemo` 的对比**：

	    | 特性 | `::AIMemo` | `::AIMemo+` |
	    |------|-----------|-------------|
	    | 知识库来源 | 整本日记（所有 .txt/.md 文件） | TagMemo 召回的 5×K chunks |
	    | Token 消耗 | 高（与日记总量成正比） | 低（基本恒定，仅与 K 相关） |
	    | 适用规模 | 中小型日记本（<10MB） | 任意规模（百MB至GB级） |
	    | TagMemo 感应 | ❌ | ✅ 残差金字塔 + N-hop 脉冲扩散 |
	    | 幽灵节点 (`[@tag]`) | ❌ | ✅ 自动注入为 coreTag |
	    | 截断保护 | 仅按 token 分批 | 先 TagMemo 精筛再分批 |
	    | LLM 推理质量 | 受全文噪音干扰 | 聚焦高相关 chunk，推理更精准 |

	-   **修饰符叠加规则**：
	    -   **K 倍率**：`[[A|B日记本::AIMemo+:1.5]]` 中的 `:1.5` 会乘到初筛 K 上 → `searchK = dynamicK × 5 × 1.5`。
	    -   **预设**：`[[A日记本::AIMemo+:my_preset]]` 与 `::AIMemo:my_preset` 一致，使用 `MoreAIMemoPresets/my_preset.json` 加载独立的模型/提示词配置。
	    -   **AIMemo 的"无视一切"语义保持**：`::Truncate`、`::Time`、`::Group`、`::Rerank`、`::TagMemo` 等其他修饰符在 `::AIMemo+` 启用时全部被忽略（与 `::AIMemo` 行为完全一致）。
	    -   **RoleValve 仍然有效**：`[[A|B日记本::AIMemo+::RoleValve@User>3]]` 这种角色楼层门控会优先判定，未通过则完全不召回。
	    -   **聚合语法兼容**：`|` 分隔多日记本时，每个库都会用同一个查询向量并行 TagMemo 召回，结果统一打包送入 AIMemo 模型。

	-   **缓存策略**：与 `::AIMemo` 共享 `aimemo` 缓存命名空间，但缓存键带 `|plus|sK<num>` 标记，与标准模式互不串号。同样的查询、同样的日记本组合在 30 分钟（默认 TTL）内会命中缓存直接返回，无需重复推理。

	-   **典型场景**：
	    -   **跨年度对话回溯**：`[[聊天历史日记本::AIMemo+]]` —— 数年的聊天记录无法整本喂给 LLM，但通过 TagMemo 精准召回最相关的 30-50 个片段后再做 AI 推理，可获得既精准又有逻辑深度的回忆结果。
	    -   **专业知识库辅助决策**：`[[医学文献|药学手册|临床指南日记本::AIMemo+:2.0]]` —— 跨库聚合大型知识库，让 AI 在小成本下产出多视角、有因果链的综合分析。
	    -   **代码库语义检索**：`[[代码片段日记本::AIMemo+]]` —— 当代码 chunk 数量上万时，先用 TagMemo 找到 5×K 个最相关片段，再让 AI 综合理解功能脉络。

	-   **何时选择 `::AIMemo` vs `::AIMemo+`**：
	    -   日记本总量 **< 10MB** → 直接用 `::AIMemo`，让 AI 看完整上下文。
	    -   日记本总量 **10MB - 100MB** → 推荐 `::AIMemo+`，平衡速度与精度。
	    -   日记本总量 **> 100MB** 或跨多库聚合 → **必须用 `::AIMemo+`**，避免 token 爆炸和分批延迟累积。
